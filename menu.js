...
{ text: `➳ *L O A D I N G .*`, timeout: 200 },
{ text: `➳ *L O A D I N G*`, timeout: 200 },
{ text: `➳ *W E L C O M E  T O  A E T H E R*`, timeout: 200 },
...
let all = await Func.Styles(`HI @${m.sender.split("@")[0]} I am ᴀᴇᴛʜᴇʀ, I am a WhatsApp assistant who can help you with certain needs\n\!` + "`☢️PROFILE NOXZ☢️`" + `\n> • Name: ${namebot}\n> • Version: ver ${version} ( Latest )\n> • Uptime: ${Func.toTime(process.uptime() * 1000)}\n> • usedPrefix: [ ${usedPrefix} ]\n> • Status: ${global.opts['self'] ? 'Self' : 'Public'} > • Clock: [ ${await DateNow(new Date)} ]\n> • Count user: ${Object.keys(db.data.users).length}\n> • Category Length: ${Object.keys(tagCount).length}\n\n` + allTagsAndHelp + "\n" + wm);
    const pp = await conn.profilePictureUrl(m.sender, 'image').catch((_) => "https://telegra.ph/file/e1b2d7c0ac4838ebd20d1.jpg");
if (global.menu === 'simple') {
  m.reply(all);
} else if (global.menu === 'doc') {
 conn.sendMessage(m.chat, {
